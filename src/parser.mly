...
%token <Lang.symbol_token> BIGGER		(* > *)
...
exp:
  ...
  | t=MINUS f=FLOAT %prec UMINUS
    { {value=EFloat {value= -. f.value; pos=t.pos};pos=t.pos} }
  | t=MINUS n=INT %prec UMINUS
  ...
  | t=FUN s=VAR OUTPUT e=exp
    { {value=EFun(s, e); pos=t.pos} }
