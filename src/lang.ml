...
let rec step (e:exp) : expression = begin
  (* print_endline (string_of_exp e.value); *)
  match e.value with
  | EInt n                    -> EInt n
  | EFloat f                  -> EFloat f
  | EVar x                    -> let error_msg = (locate e.pos) ^ "Unbound value: " ^ x.value in failwith error_msg
  | EAdd (e1, e2)             ->
    if not (is_value e1.value) then EAdd({value=step e1;pos=e1.pos}, e2) 
    else if not (is_value e2.value) then EAdd(e1, {value=step e2;pos=e2.pos})
    else eval_num e1 PLUS e2
  | ESubtract (e1, e2)        ->
    if not (is_value e1.value) then ESubtract({value=step e1;pos=e1.pos}, e2)
    else if not (is_value e2.value) then ESubtract(e1, {value=step e2;pos=e2.pos})
    else eval_num e1 MINUS e2
  | EMultiply (e1, e2)        -> 
    if not (is_value e1.value) then EMultiply({value=step e1;pos=e1.pos}, e2)
    else if not (is_value e2.value) then EMultiply(e1, {value=step e2;pos=e2.pos})
    else eval_num e1 TIMES e2
  | EDivide (e1, e2)          -> 
    if not (is_value e1.value) then EDivide({value=step e1;pos=e1.pos}, e2)
    else if not (is_value e2.value) then EDivide(e1, {value=step e2;pos=e2.pos})
    else eval_num e1 DIVIDE e2
  | EBool b                   -> EBool b
  | ELessthanorequal (e1, e2) -> 
    if not (is_value e1.value) then ELessthanorequal({value=step e1;pos=e1.pos}, e2)
    else if not (is_value e2.value) then ELessthanorequal(e1, {value=step e2;pos=e2.pos})
    else eval_num e1 SMALLEREQUAL e2
  | EMorethan (e1, e2) ->
    if not (is_value e1.value) then EMorethan({value=step e1;pos=e1.pos}, e2)
    else if not (is_value e2.value) then EMorethan(e1, {value=step e2;pos=e2.pos})
    else eval_num e1 BIGGER e2
  | EIf (e1, e2, e3)          ->
    if not (is_value e1.value) then EIf({value=step e1;pos=e1.pos}, e2, e3)
    else begin
    match e1.value with
    | EBool b -> if b.value then (step e2) else (step e3)
  ...
  | ELet(s, e1, e2)           ->
    if (is_value e1.value) then step (substitute e1 s.value e2)
    else ELet(s, {value=step e1;pos=e1.pos}, e2)
  | EFun(s, e')               -> EFun (s, e')
  | EFix(f, x, e')            -> EFix (f, x, e')
  | EApply (e1, e2)           ->
    if not (is_value e2.value) then EApply (e1, {value=step e2;pos=e2.pos})
    else if not (is_value e1.value) then EApply ({value=step e1;pos=e1.pos}, e2)
    else
    match e1.value with
    | EFun(s, e') -> step (substitute e2 s.value e')
    | EFix(f, x, e') -> step (substitute e1 f.value (substitute e2 x.value e'))
...
(* The evaluation function that calls step *)
   ...
   let v = step e
   in
   if b then print_endline (string_of_exp v);
   if (is_value v) then (exp_to_value v)
