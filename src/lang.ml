...
let rec step (e:exp) (env:environment) (b:bool) : environment * expression =
...
| EHead e                                     -> 
  if not (is_value e.value) then let eval = step e env false in (fst eval, EHead {value=snd eval; pos=e.pos})
  else begin 
  match e.value with
  ...
  | _             -> let error_msg = (locate e.pos) ^ "hd can only operate on lists" in failwith error_msg end
| ETail e                                     -> 
  if not (is_value e.value) then let eval = step e env false in (fst eval, ETail {value=snd eval; pos=e.pos})
  else begin 
  match e.value with
  ...
  | _             -> let error_msg = (locate e.pos) ^ "tl can only operate on lists" in failwith error_msg end
| EEmpty e                                    -> 
  if not (is_value e.value) then let eval = step e env false in (fst eval, EEmpty {value=snd eval; pos=e.pos})
  else begin 
  match e.value with
...
| ECons(e1, e2)                               ->
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, ECons ({value=snd eval1; pos=e.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, ECons (e1, {value=snd eval2; pos=e.pos}))
     else (env, ECons(e1, e2))  
| EPair(e1, e2)                               ->
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EPair({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EPair(e1, {value=snd eval2; pos=e2.pos}))
     else (env, EPair(e1, e2))
| EFst e                                      ->
  if not (is_value e.value) then let eval = step e env false in (fst eval, EFst {value=snd eval; pos=e.pos}) else begin 
  match e.value with
  | EPair(e1, e2) -> (env, e1.value)
  | _             -> let error_msg = (locate e.pos) ^ "fst can only operate on pair expressions" in failwith error_msg end
| ESnd e                                      ->
  if not (is_value e.value) then let eval = step e env false in (fst eval, ESnd {value=snd eval; pos=e.pos}) else begin
  match e.value with
  | EPair(e1, e2) -> (env, e2.value)
  | _             -> let error_msg = (locate e.pos) ^ "snd can only operate on pair expressions" in failwith error_msg end
| ERef e                                      -> let eval = ref (step e env false) in
                                                 while not (is_value (snd !eval)) do eval := step {value=snd !eval; pos=e.pos} (fst !eval) false done ; 
                                                 let n = List.length env in ((n, (snd !eval)) :: env, EPointer n) (* We want to treat ref as a value *)
| EPointer n                                  -> (env, e.value)
| EUpdate(e1, e2)                             -> if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EUpdate({value=snd eval1; pos=e1.pos}, e2))
                                                 else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EUpdate(e1, {value=snd eval2; pos=e2.pos}))
                                                 else begin
                                                 match e1.value with
...
| EDeref e                                    -> if not (is_value e.value) then let eval = step e env false in (fst eval, EDeref {value=snd eval; pos=e.pos})
                                                 else begin
                                                 match e.value with
                                                 | EPointer n     -> (env, List.assoc n env)
                                                 | _              -> let error_msg = (locate e.pos) ^ "Nothing to dereference" in failwith error_msg end

| EIgnore(e1, e2)                             -> if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EIgnore({value=snd eval1; pos=e1.pos}, e2))
                                                 else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EIgnore(e1, {value=snd eval2; pos=e2.pos}))
                                                 else (env, e2.value)
| EWhile(e1, e2)                              -> let eval1 = ref (step e1 env false) in
                                                 while not (is_value (snd !eval1)) do eval1 := step {value=snd !eval1; pos=e1.pos} (fst !eval1) false done;
                                                 begin match (snd !eval1) with
                                                 | EBool b' -> if b'.value then let eval2 = ref (step e2 (fst !eval1) b) in
                                                               while not (is_value (snd !eval2)) do eval2 := step {value=snd !eval2; pos=e2.pos} (fst !eval2) b done ; ((fst !eval2), EWhile(e1, e2))
                                                               else (env, EUnit)
                                                 | _        -> failwith "This is not a boolean expression" end
| EAdd (e1, e2)                               ->
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EAdd({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EAdd(e1, {value=snd eval2;pos=e2.pos}))
     else begin
     match e1.value with
...
| ESubtract (e1, e2)                          ->
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, ESubtract({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, ESubtract(e1, {value=snd eval2;pos=e2.pos}))
     else begin
     match e1.value with
     | EPointer n1 -> begin match e2.value with
...
| EMultiply (e1, e2)                          -> 
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EMultiply({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EMultiply(e1, {value=snd eval2;pos=e2.pos}))
     else  begin
     match e1.value with
     | EPointer n1 -> begin match e2.value with
...
| EDivide (e1, e2)                            -> 
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EDivide({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EDivide(e1, {value=snd eval2;pos=e2.pos}))
     else begin
     match e1.value with
     | EPointer n1 -> begin match e2.value with
...
| ELessthanorequal (e1, e2)                   -> 
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, ELessthanorequal({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, ELessthanorequal(e1, {value=snd eval2;pos=e2.pos}))
     else begin
     match e1.value with
     | EPointer n1 -> begin match e2.value with
...
| EMorethan (e1, e2)                          ->
     if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EMorethan({value=snd eval1; pos=e1.pos}, e2))
     else if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EMorethan(e1, {value=snd eval2;pos=e2.pos}))
     else begin
     match e1.value with
     | EPointer n1 -> begin match e2.value with
...
| EIf (e1, e2, e3)                            ->
     if not (is_value e1.value) then let eval = step e1 env false in (fst eval, EIf({value=snd eval;pos=e1.pos}, e2, e3))
     else begin
     match e1.value with 
     | EBool b' ->  if b'.value then (step e2 env b) else (step e3 env b)
     | _        ->  let error_msg = (locate e1.pos) ^ "This is not a boolean expression." in failwith error_msg end
| ELet(s, e1, e2)                             ->
     if not (is_value e1.value) then let eval = step e1 env false in (fst eval, ELet(s, {value=snd eval;pos=e1.pos}, e2))
     else step (substitute e1 s.v.value e2) env b
| EFun(s, e')                                 -> (env, EFun (s, e'))
| EFix(f, x, e')                              -> (env, EFix (f, x, e'))
| EApply (e1, e2)                             -> 
     if not (is_value e2.value) then let eval2 = step e2 env false in (fst eval2, EApply (e1, {value=snd eval2;pos=e2.pos}))
     else if not (is_value e1.value) then let eval1 = step e1 env false in (fst eval1, EApply ({value=snd eval1;pos=e1.pos}, e2))
     else begin
     match e1.value with
     | EFun(s, e') -> step (substitute e2 s.v.v.value e') env b
     | EFix(f, x, e') -> step (substitute e1 f.value (substitute e2 x.v.v.value e')) env b
...
let eval (e:exp) (env:environment) (b:bool) : value =
  let rec eval_H (envmt:environment) (e:exp) : value =
    let v = step e envmt b
    in
    if (is_value (snd v)) then begin print_endline (string_of_exp (snd v)) ; (exp_to_value (fst v) (snd v)) end
    else eval_H (fst v) {value=(snd v); pos=e.pos}
    in eval_H env e
